...
function! s:conf(name, default)
  ...
  return get(conf, a:name, get(g:, 'tug_' . a:name, a:default))
endfunction
...
function! s:check_requirements()
  ...
  if !exists('*tug#run')
    throw "tug#run function not found. You also need Vim plugin from the main tug repository (i.e. khulnasoft/tug *and* khulnasoft/tug.vim)"
  endif
  if !exists('*tug#exec')
    throw "tug#exec function not found. You need to upgrade Vim plugin from the main tug repository ('khulnasoft/tug')"
  endif
  let s:checked = !empty(tug#exec(s:min_version))
endfunction
...
function! s:extend_opts(dict, eopts, prepend)
  ...
  if has_key(a:dict, 'options')
    if type(a:dict.options) == s:TYPE.list && type(a:eopts) == s:TYPE.list
      ...
      let a:dict.options = join(map(all_opts, 'type(v:val) == s:TYPE.list ? join(map(copy(v:val), "tug#shellescape(v:val)")) : v:val'))
    endif
  else
...
function! s:reverse_list(opts)
  let tokens = map(split($TUG_DEFAULT_OPTS, '[^a-z-]'), 'substitute(v:val, "^--", "", "")')
...
function! s:wrap(name, opts, bang)
  " tug#wrap does not append --expect if sink or sink* is found
  ...
  if options !~ '--expect' && has_key(opts, 'sink*')
    ...
    let wrapped = tug#wrap(a:name, opts, a:bang)
...
function! s:tug(name, opts, extra)
  ...
  return tug#run(s:wrap(a:name, merged, bang))
endfunction
...
function! s:action_for(key, ...)
  let Cmd = get(get(g:, 'tug_action', s:default_action), a:key, '')
...
function! s:fill_quickfix(name, list)
  if len(a:list) > 1
    let Handler = s:conf('listproc_'.a:name, s:conf('listproc', function('tug#vim#listproc#quickfix')))
...
function! tug#vim#lines(...)
  let [display_bufnames, lines] = tug#vim#_lines(1)
...
function! tug#vim#colors(...)
  let colors = split(globpath(&rtp, "colors/*.vim"), "\n")
  if has('packages')
    let colors += split(globpath(&packpath, "pack/*/opt/*/colors/*.vim"), "\n")
  endif
  let colors = tug#vim#_uniq(map(colors, "fnamemodify(v:val, ':t')[:-5]"))
  ...
  if !a:1 " We can't set up IPC in fullscreen mode in Vim
    let fifo = tug#vim#ipc#start({ msg -> execute('colo '.msg) })
    if len(fifo)
  ...
  call s:tug('colors', spec, a:000)
endfunction

" ------------------------------------------------------------------
" Locate
" ------------------------------------------------------------------
function! tug#vim#locate(query, ...)
  return s:tug('locate', {
...
" History[:/]
" ------------------------------------------------------------------
function! tug#vim#_recent_files()
  return tug#vim#_uniq(map(
    \ filter([expand('%')], 'len(v:val)')
    \   + filter(map(tug#vim#_buflisted_sorted(), 'bufname(v:val)'), 'len(v:val)')
...
nnoremap <plug>(-tug-vim-do) :execute g:__tug_command<cr>
nnoremap <plug>(-tug-/) /
nnoremap <plug>(-tug-:) :
...
function! s:history_sink(type, lines)
  ...
  if key == 'ctrl-e'
    ...
    let g:__tug_command = "normal ".prefix.item."\<cr>"
    call feedkeys("\<plug>(-tug-vim-do)")
  endif
endfunction
...
function! tug#vim#command_history(...)
  return s:tug('history-command', {
...
function! tug#vim#search_history(...)
  return s:tug('history-search', {
  \ 'source':  s:history_source('/'),
  \ 'sink*':   s:function('s:search_history_sink'),
  \ 'options': '+m --ansi --prompt="Hist/> " --header-lines=1 --expect=ctrl-e --tiebreak=index'}, a:000)
endfunction

function! tug#vim#history(...)
  return s:tug('history-files', {
  \ 'source':  tug#vim#_recent_files(),
...
function! tug#vim#gitfiles(args, ...)
  let dir = get(get(a:, 1, {}), 'dir', '')
  let root = s:get_git_root(dir)
  if empty(root)
    return s:warn('Not in git repo')
  endif
  let prefix = 'git -C ' . tug#shellescape(root) . ' '
  if a:args != '?'
  ...
  " Here be dragons!
  " We're trying to access the common sink function that tug#wrap injects to
  ...
  let preview = printf(
    ...
    \ executable('delta')
      \ ? diff_prefix . 'diff -- {-1} ' . bar . ' delta --width $TUG_PREVIEW_COLUMNS --file-style=omit ' . bar . ' sed 1d'
      \ : diff_prefix . 'diff --color=always -- {-1} ' . bar . ' sed 1,4d',
    \ s:escape_for_bash(s:bin.preview))
  let wrapped = tug#wrap({
  ...
  return s:tug('gfiles-diff', wrapped, a:000)
endfunction
...
function! tug#vim#_format_buffer(b)
...
function! s:sort_buffers(...)
  let [b1, b2] = map(copy(a:000), 'get(g:tug#vim#buffers, v:val, v:val)')
  " Using minus between a float and a number in a sort function causes an error
  return b1 < b2 ? 1 : -1
endfunction

function! tug#vim#_buflisted_sorted()
  return sort(s:buflisted(), 's:sort_buffers')
endfunction

" [query (string)], [bufnrs (list)], [spec (dict)], [fullscreen (bool)]
function! tug#vim#buffers(...)
  let [query, args] = (a:0 && type(a:1) == type('')) ?
        \ [a:1, a:000[1:]] : ['', a:000]
  ...
  return s:tug('buffers', {
  \ 'source':  map(sorted, 'tug#vim#_format_buffer(v:val)'),
...
function! tug#vim#ag(query, ...)
  ...
  let command = ag_opts . ' -- ' . tug#shellescape(query)
  return call('tug#vim#ag_raw', insert(args, command, 0))
endfunction

" ag command suffix, [spec (dict)], [fullscreen (bool)]
function! tug#vim#ag_raw(command_suffix, ...)
  if !executable('ag')
    return s:warn('ag is not found')
  endif
  return call('tug#vim#grep', extend(['ag --nogroup --column --color '.a:command_suffix, 1], a:000))
endfunction
...
function! tug#vim#grep(grep_command, ...)
  let args = copy(a:000)
  let words = []
  ...
  try
    let prev_default_command = $TUG_DEFAULT_COMMAND
    let $TUG_DEFAULT_COMMAND = command
    return s:tug(name, opts, args)
  finally
    let $TUG_DEFAULT_COMMAND = prev_default_command
  endtry
endfunction


" command_prefix (string), initial_query (string), [spec (dict)], [fullscreen (bool)]
function! tug#vim#grep2(command_prefix, query, ...)
  let args = copy(a:000)
  let words = []
  ...
  call extend(opts.options, ['--bind', 'start:reload:'.a:command_prefix.' '.tug#shellescape(a:query).suffix])
...
function! tug#vim#buffer_tags(query, ...)
  let args = copy(a:000)
  let escaped = tug#shellescape(expand('%'))
...
function! tug#vim#tags(query, ...)
  ...
  let args = insert(map(tagfiles, 'tug#shellescape(fnamemodify(v:val, ":p"))'), tug#shellescape(a:query), 0)
  return s:tug('tags', {
  \ 'source':  join(['perl', tug#shellescape(s:bin.tags), join(args)]),
...
function! tug#vim#commands(...)
  redir => cout
  silent command
  redir END
  let list = split(cout, "\n")
  return s:tug('commands', {
...
function! tug#vim#changes(...)
  let all_changes = ["buf  offset  line  col  text"]
  let cursor = 0
  for bufnr in tug#vim#_buflisted_sorted()
...
function! tug#vim#helptags(...)
  ...
  return s:tug('helptags', {
  \ 'source': 'perl '.tug#shellescape(s:helptags_script).' '.join(map(tags, 'tug#shellescape(v:val)')),
...
" File types
" ------------------------------------------------------------------
function! tug#vim#filetypes(...)
  return s:tug('filetypes', {
  \ 'source':  tug#vim#_uniq(sort(map(split(globpath(&rtp, 'syntax/*.vim'), '\n'),
...
function! s:commits_sink(lines)
  ...
  let Cmd = get(get(g:, 'tug_action', s:default_action), a:lines[0], '')
  let cmd = type(Cmd) == s:TYPE.string ? Cmd : ''
...
function! s:commits(range, buffer_local, args)
  ...
  let prefix = 'git -C ' . tug#shellescape(s:git_root) . ' '
  let source = prefix . 'log '.s:conf('commits_log_options', '--color=always '.tug#shellescape('--format=%C(auto)%h%d %s %C(green)%cr'))
  let current = expand('%:p')
  let managed = 0
  if !empty(current)
    call system(prefix . 'show '.tug#shellescape(current).' 2> '.(s:is_win ? 'nul' : '/dev/null'))
  ...
  if len(a:range) || a:buffer_local
    ...
    if len(a:range)
      let source .= join([printf(' -L %d,%d:%s --no-patch', a:range[0], a:range[1], tug#shellescape(current)), log_opts])
      if with_preview
        let previewparams = join([printf('log -L %d,%d:%s', a:range[0], a:range[1], tug#shellescape(current)), log_opts])
      ...
      let source .= join([' --follow', log_opts, tug#shellescape(current)])
    endif
  ...
  let expect_keys = join(keys(get(g:, 'tug_action', s:default_action)), ',')
  let options = {
  \ 'source':  source,
  ...
  if with_preview
    if !len(a:range)
      ...
      let previewcmd = 'echo {} | grep -o "[a-f0-9]\{7,\}" | head -1 | xargs ' . prefix . 'show -O'.tug#shellescape(orderfile).' --format=format: --color=always '
    endif
    let suffix = executable('delta') ? '| delta --width $TUG_PREVIEW_COLUMNS' : ''
    call extend(options.options, ['--preview', previewcmd . suffix])
  endif

  return s:tug(a:buffer_local ? 'bcommits' : 'commits', options, args)
endfunction
...
function! tug#vim#commits(...) range
  if exists('b:tug_winview')
    call winrestview(b:tug_winview)
    unlet b:tug_winview
  endif
...
" [git-log-args], [spec (dict)], [fullscreen (bool)]
function! tug#vim#buffer_commits(...) range
  if exists('b:tug_winview')
    call winrestview(b:tug_winview)
    unlet b:tug_winview
  endif
...
" ------------------------------------------------------------------
" tug#vim#maps(mode, opts[with count and op])
" ------------------------------------------------------------------
...
" ----------------------------------------------------------------------------
" tug#vim#complete - completion helper
" ----------------------------------------------------------------------------
inoremap <silent> <Plug>(-tug-complete-trigger) <c-o>:call <sid>complete_trigger()<cr>
...
function! s:complete_insert(lines)
  ...
  elseif has('nvim')
    ...
    call feedkeys("\<Plug>(-tug-complete-finish)")
  endif
endfunction

nnoremap <silent> <Plug>(-tug-complete-finish) a
inoremap <silent> <Plug>(-tug-complete-finish) <c-o>l
...
function! tug#vim#complete(...)
  if a:0 == 0
    let s:opts = tug#wrap()
  elseif type(a:1) == s:TYPE.dict
    let s:opts = copy(a:1)
  elseif type(a:1) == s:TYPE.string
    let s:opts = extend({'source': a:1}, get(a:000, 1, tug#wrap()))
  else
  ...
  call feedkeys("\<Plug>(-tug-complete-trigger)")
  return ''
endfunction

...
START_LINE="$(( CENTER - TUG_PREVIEW_LINES / 2 ))"
if (( START_LINE <= 0 )); then
    START_LINE=1
fi
END_LINE="$(( START_LINE + TUG_PREVIEW_LINES - 1 ))"
...
if [ -z "$TUG_PREVIEW_COMMAND" ] && [ "${BATNAME:+x}" ]; then
  ${BATNAME} --style="${BAT_STYLE:-numbers}" \
             --color=always \
             --pager=never \
             --wrap=never \
             --terminal-width="${TUG_PREVIEW_COLUMNS}" \
...
CMD=${TUG_PREVIEW_COMMAND:-$DEFAULT_COMMAND}
